# Modified from CSDN 陈君豪：4自由度机械臂正逆解公式推导与代码实现
# https://blog.csdn.net/howard789/article/details/122120720
...
def test_ok(x,y,z,alpha, a6=90):
    ...
    if valid:
        ...
        if abs(x1-x)>0.5 or abs(y1-y)>0.5 or abs(z1-z)>0.5:
            ...
            # 创建机械臂对象
            time.sleep(1)
            Arm.Arm_serial_servo_write6(deg1, deg2, deg3, deg4, 90, a6, 1000)
            time.sleep(1)
            Arm.Arm_serial_servo_write6(deg1, deg2, deg3, deg4, 90, 150, 1000)
...
if __name__ == '__main__':
    ...
    z = 2
    alpha=180 # 向上为0
    test_ok(x,y,z, alpha, 90)
    time.sleep(5)
    x = 0
    y = 20
    z = 20
    test_ok(x,y,z, alpha, 150)